...
# A2UI Demo - Project Custom Rules (Override tasteskill defaults)

## Project Context
- This is a **chat-based course catalog agent**, NOT a landing page or portfolio
- Primary UI: conversational interface with real-time streaming text
- A2UI components render inside `<A2uiSurface>` wrappers (from `@a2ui/react`)
- Course cards are AI-generated by the agent, not hand-coded by the developer
- Backend: Python FastAPI + LangGraph + OpenAI GPT-4o
- Frontend: Next.js 15 + React 19 + Tailwind CSS v4 + TypeScript

## Dial Overrides (for this project)
- `DESIGN_VARIANCE: 5` (chat app, not asymmetric landing page)
- `MOTION_INTENSITY: 4` (subtle reveals, no scroll hijack)
- `VISUAL_DENSITY: 3` (spacious message area)

## A2UI-Specific Rules
- Style `<A2uiSurface>` containers with diffused shadows and rounded corners
- Course cards use double-bezel architecture (outer shell + inner core)
- Streaming text gets a subtle emerald pulse cursor
- Tool status shows as bouncing dots, not generic spinners
- Chat messages animate in with `fade-in-up` on entry
- Input field uses pill shape with glow focus ring
- Send button is round with spring hover physics

## Tech Stack Constraints
- Use Tailwind CSS v4 (already configured with `@tailwindcss/postcss`)
- Use `next/font` for font loading (Geist + Geist Mono already configured)
- Use CSS custom properties for design tokens (defined in `globals.css`)
- Dark mode via `prefers-color-scheme` media query (already implemented)
- No additional UI libraries needed - pure Tailwind + CSS variables

## Files to Reference
- `frontend/src/app/globals.css` - Design tokens, animations, glass effects
- `frontend/src/app/layout.tsx` - Font setup, noise overlay
- `frontend/src/components/a2ui/A2UIChat.tsx` - Main chat interface
- `frontend/src/lib/a2ui-catalog.tsx` - Course card component
