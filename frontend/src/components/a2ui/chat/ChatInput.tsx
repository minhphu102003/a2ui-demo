...
export function ChatInput({
  ...
  return (
    <footer className="glass-heavy border-t border-[var(--color-border-subtle)] w-full">
      <div className="max-w-5xl mx-auto px-6 py-4 sm:px-10 sm:py-5">
        <div className="flex items-center gap-3">
          <div className="flex-1 relative">
            <input
              ...
              disabled={isLoading}
              className="w-full h-12 px-5 rounded-[var(--radius-full)] bg-[var(--color-surface)] border border-[var(--color-border)] text-[15px] text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-accent)] focus:shadow-[var(--shadow-glow)] transition-all duration-[var(--duration-normal)] ease-[var(--ease-smooth)] disabled:opacity-50"
            />
          </div>
          <button
            onClick={onSend}
            disabled={isLoading || !input.trim()}
            className="group flex items-center justify-center w-12 h-12 shrink-0 rounded-[var(--radius-full)] bg-[var(--color-accent)] text-white shadow-[var(--shadow-lifted)] hover:bg-[var(--color-accent-hover)] hover:shadow-[var(--shadow-glow)] active:scale-[0.95] disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-[var(--color-accent)] disabled:hover:shadow-[var(--shadow-lifted)] transition-all duration-[var(--duration-normal)] ease-[var(--ease-spring)]"
          >
            {isLoading ? (
